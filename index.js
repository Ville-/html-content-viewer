var HTMLContentViewer = {
  init: function (settings) {
    ...
    this.settings = {
      ...
      CSSURL: settings.CSSURL ? settings.CSSURL : null,
      containerClass: settings.containerClass ? settings.containerClass : '',
    };
  },

  showHTMLContentInNewWindow: function (content, title = '') {
    if (!this.settings) throw new Error('Run HTMLContentViewer.init first');

    var printWindow = openWindow(this.settings.height, this.settings.width);

    printWindow.document.write('<html><head>');
    printWindow.document.write(getTitle(title));
    printWindow.document.write(getStyles(this.settings.CSSURL));
    ...
    printWindow.document.write(getContent(content, this.settings.containerClass));
...
function getUIComponents(showPrintButton, showCloseButton, printButtonText, closeButtonText) {
  ...
  if (showPrintButton) {
    ui += '<a href="#" class="html-content-viewer-btn" onclick="printContent()">' + printButtonText + '</a>';
  }

  if (showCloseButton) {
    ui += '<a href="#" class="html-content-viewer-btn" onclick="closeWindow()">' + closeButtonText + '</a>';
  }
...
function getContent(content, containerClass) {
...
function getStyles(CSSURL) {
  ...
  if (CSSURL) {
    styles += '<link rel="stylesheet" type="text/css" href="' + CSSURL + '">';
  }
